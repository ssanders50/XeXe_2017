...
void EPCalib(unsigned int minRun=0, unsigned int maxRun=500000){
  ...
  save = fopen("/rfs/sanders/tmpEPCalib","wb");
  int Hbins = 0;
  int Obins = 0;
  char buf[200];
  FILE * list;
  ...
  fclose(list);
  TFile * tfout = new TFile("/rfs/sanders/EP.root","recreate");
  ...
  for(int findx = 0; findx< lcnt; findx++) {
	...
	string name;
	if(useNtrk) {
	  name = "evtPlaneCalibTree/"+to_string(trkBins[i])+"_"+to_string(trkBins[i+1]);
	} else {
  ...
  TFile * foff = new TFile("foff.root","recreate");
  foff->cd();
  for(int i = 0; i<mx; i++) {
    for(int j = 1; j<=7; j++){
