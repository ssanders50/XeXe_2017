...
HiEvtPlaneFlatCalib::HiEvtPlaneFlatCalib(const edm::ParameterSet& iConfig):runno_(0)
{
  ...
  epnames = epnames+"/D";
  hPhiEta0 = fs->make<TH2F>("PhiEta0","PhiEta0",100, -3, 3, 100, -4, 4);
  hPhiEta0->SetOption("colz");
  hPhiEta0->SetXTitle("Eta");
  hPhiEta0->SetYTitle("Phi");

  hPhiEta1 = fs->make<TH2F>("PhiEta1","PhiEta1",100, -3, 3, 100, -4, 4);
  hPhiEta1->SetOption("colz");
  hPhiEta1->SetXTitle("Eta");
  hPhiEta1->SetYTitle("Phi");

  hPhiEta2 = fs->make<TH2F>("PhiEta2","PhiEta2",100, -3, 3, 100, -4, 4);
  hPhiEta2->SetOption("colz");
  hPhiEta2->SetXTitle("Eta");
  hPhiEta2->SetYTitle("Phi");

  hPhiEta3 = fs->make<TH2F>("PhiEta3","PhiEta3",100, -3, 3, 100, -4, 4);
  hPhiEta3->SetOption("colz");
  hPhiEta3->SetXTitle("Eta");
  hPhiEta3->SetYTitle("Phi");

  hPhiEta4 = fs->make<TH2F>("PhiEta4","PhiEta4",100, -3, 3, 100, -4, 4);
  hPhiEta4->SetOption("colz");
  hPhiEta4->SetXTitle("Eta");
  hPhiEta4->SetYTitle("Phi");

  hPhiEta5 = fs->make<TH2F>("PhiEta5","PhiEta5",100, -3, 3, 100, -4, 4);
  hPhiEta5->SetOption("colz");
  hPhiEta5->SetXTitle("Eta");
  hPhiEta5->SetYTitle("Phi");
...
HiEvtPlaneFlatCalib::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(trackCollection_.isValid()){
	...
	vyErr=vertex_->begin()->yError();
      ...
      bool accepted = true;
      if(j->charge()!=0&&accepted) hPhiEta0->Fill(j->eta(), j->phi());
      bool isPixel = false;
	...
	hpixelTrack->Fill(1);
      ...
      if(j->charge()!=0&&accepted) hPhiEta1->Fill(j->eta(), j->phi());
      if ( ! isPixel){
	if ( fabs(dz/dzsigma) > 3 ) accepted = false;
	if(j->charge()!=0&&accepted) hPhiEta2->Fill(j->eta(), j->phi());
	if ( fabs(d0/d0sigma) > 3 ) accepted = false;
	if(j->charge()!=0&&accepted) hPhiEta3->Fill(j->eta(), j->phi());
	if ( j->ptError()/j->pt() > 0.1 ) accepted = false;
	if(j->charge()!=0&&accepted) hPhiEta4->Fill(j->eta(), j->phi());
	if ( j->numberOfValidHits() < 12 ) accepted = false;
	if(j->charge()!=0&&accepted) hPhiEta5->Fill(j->eta(), j->phi());
	hpixelTrack->Fill(2);
      }
      if( accepted ) {
	track_eta = j->eta();
  ...
  for (EvtPlaneCollection::const_iterator rp = inputPlanes_->begin();rp !=inputPlanes_->end(); rp++) {
	...
	cout<<"DATA INCONSISTENCY.  Too many or wrong EPs found in collection. Expect a crash!"<<endl;
      ...
      if(useOffsetPsi_) {
	psiOffset = flat[i]->getOffsetPsi(flat[i]->getSoffset(s,vtx,bin),flat[i]->getCoffset(c,vtx,bin));
	//if(i==NumEPNames-2) cout<<EPNames[i].data()<<"\t"<<angorig<<"\t"<<psiOffset<<"\t"<<indx<<"\t"<<c<<"\t"<<flat[i]->getXoffDB(indx)<<"\t"<<s<<"\t"<<flat[i]->getYoffDB(indx)<<"\t"<<flat[i]->getEtScale(vtx,bin)<<"\t"<<flat[i]->getPtDB(indx)<<"\t"<<flat[i]->getPt2DB(indx)<<endl;
	epsin[i] = flat[i]->getSoffset(s,vtx,bin);
	epcos[i] = flat[i]->getCoffset(c,vtx,bin);
	if(w>0) {
	  epqx[i]  = flat[i]->getCoffset(c,vtx,bin)/w; 
	  epqy[i]  = flat[i]->getSoffset(s,vtx,bin)/w;
	  epq[i]   = ((pow(epqx[i],2)+pow(epqy[i],2))>0)? sqrt(pow(epqx[i],2)+pow(epqy[i],2)): 0.;};
      }
      ...
      if(genFlatPsi_) {
	if(bin>=0&&abs(vtx)<plotZrange&&centval<80) {
